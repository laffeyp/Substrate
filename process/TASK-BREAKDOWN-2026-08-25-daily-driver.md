...
| 210 | OBSERVATION — piece A end-to-end | Fire the piece-A observation contract from TECH-SPEC §3: script + expected log substrings + expected runtime signals + expected screenshot. Behavior-touching, no new source; runs the harness. | `tests/test_session_topology_e2e.py`, `tests/fixtures/two_turns.json` | Reads: piece-A output on `records/`. |
