...
| Shared best-of-N + correction contract: `Draft`, `Candidate`, `Verdict`, `Solved`, `Exhausted` (+ `ModelUsage`) | `substrate/topologies/coding_flow/__init__.py` | **3-CONSUMER shared contract** (Wave-0 #15) — coding_flow, the swebench_solver Repairer, and code_evolution all consume ONE set; reused, not re-rolled (review #57 / KIT_DIARY finding 12). Sub-topology extracted in sprint 134-series; the terminal/collect-vs-stop policy is a per-consumer nested-topology parameter, NOT a record change. Sprint 133. |
| swebench_solver records: `SuspectFiles`, `SuspectElements`, `EditLocations`, `AppliedPatch`, `TestResults`, `SelectedPatch` | `substrate/topologies/swebench_solver/` (planned) | The LOCALIZE + SELECT records wrapping the shared loop. Fields locked in `process/signals/swebench-solver-vocabulary.md` (sprint 133). `TestResults` is `replayable=False` (run-and-observe Docker seam); the SELECT regression set is repo-derived, NOT the `PASS_TO_PASS` grade field (firewall). |
