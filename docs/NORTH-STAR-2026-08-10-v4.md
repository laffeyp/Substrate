# NORTH STAR — substrate as the cockpit (2026-08-10, v4)

*Fifth version. Prior four on disk as the audit trail:
`NORTH-STAR-2026-08-09.md` (v1 — long, richer texture, marketing framing);
`NORTH-STAR-2026-08-10.md` (v2 — spine on five themes);
`NORTH-STAR-2026-08-10-v2.md` (v3 pass 1 — trimmed, some vocabulary scrubbed);
`NORTH-STAR-2026-08-10-v3.md` (v3 pass 2 — scrubbed further, went flat). This
version restores the weight the trimming took out and keeps the corrected register.*
...
the whole factored setup — the models are already put where they belong, the roles
already known, the context already threaded. This shape is close in spirit to the
...
strong-model-plus-verifier shape is still the sensible default.
...
name and gets the whole setup, not a fresh best-effort interpretation of a vague
ask. This is where the toolbox becomes usable. It is where substrate stops being a
kit and starts being a tool.
...
tomorrow is a specialist ensemble under a coordinator. The runtime does not care
which; the strategy definitions do not name the model, only the role.
...
against the substrate record so caching is a runtime concern, not a per-request
LLM concern. The strong model sits at the orchestrator position; small models
