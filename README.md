...
## What it is
Substrate is a way of working with LLM's that happens to do much more. If you are familiar with Codex, Claude, Amp, etc - they all combine an LLM, that can only input and output text - with a suite of tools and procedures that determine how the stochastic text generated by by what is essentially an extremely high dimensionality probablity space encoding relationships between elements of languange and all of human creative output that was on the internet when they trained the model, how that text can usefully direct work on a computer, basically by just making a kind of API that LLM's can use, in essence an orchestration system, but using open source models running on your machine or in the cloud.  Substrate allows you to build your own systems to do similar things, but in a very specific and useful weay. 

But you can also recreate whole parts of traditional software workflow - say you want to run three teams of pair coding LLM's with varying acceptance criteria across teams and rounds. And you want some external data stream to be totally able to change the direction of work if some specific conditions are met. Substrate can help you do that. 

Say you want to have LLM's play any kind of game theoretical game, or you want to do reasearch on how differnt kinds of LLM's responsd to the same kind of inputs, or you want to run some kind of adversarial critiquing process on a set of documents in order to refine them.... Substrate can help you do that. In fact, many of these applications are built and availble for you to use, and view with substrate-ui. 

We do this by drasticaly simplifying the mental model of what we are doing here. Once this this is done, key concepts emerge that we can then use as the biulding blocks of an abstract system. Substrate is that system. 
