...
async def _run(args: argparse.Namespace) -> int:
    ...
    print(f"repo={args.repo}  ref={args.ref}  roles={list(roles)}  quorum={quorum}  model={args.model}")
    ...
    for e in read_record(record):
        ...
        if k == "CritiquePosted":
            ...
            print(f"  ✓ verdict: {p['decision']}  (cited {list(p['cited_roles'])}, {p['n_critiques']} critiques)")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="review the changed files of a git diff with an N-role panel")
    ap.add_argument("--repo", required=True, help="path to a git repo to review")
    ap.add_argument("--ref", default="HEAD~1", help="diff against this ref (default: HEAD~1 — the last commit)")
    ap.add_argument("--model", default="kimi-k2.6:cloud", help="an Ollama model for every reviewer + the judge")
    ap.add_argument("--roles", nargs="*", help=f"reviewer roles (default: {list(DEFAULT_ROLES)})")
    ap.add_argument("--quorum", type=int, default=3, help="fire the judge at this many critiques (default: 3)")
