#!/usr/bin/env python3
"""Run research_sweep over a real document set with real models (walkthrough, workflow-parity W1.3).
...
all findings, then synthesize the answer. The record is replayable.
    ...
    uv run python scripts/run_research_sweep.py --dir docs/cockpit \
        --question "What is the next version of the UI?" --model kimi-k2.6:cloud

CI proves the wiring (tests/test_research_sweep.py, deterministic); this is the real-model path.
"""
...
async def _run(args: argparse.Namespace) -> int:
    ...
    topo = research_sweep_topology(
        ...
        reader=model, critic=model, synthesizer=model,
        deterministic=False,
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="fan readers over a document set, critique gaps, synthesize")
    ...
    ap.add_argument("--model", default="kimi-k2.6:cloud", help="the model for reader/critic/synthesizer")
