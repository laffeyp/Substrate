...
**DRAFT 3.** Supersedes DRAFT 2; adds §22 — the concrete technology bill of materials: every library, tool, vendored component, and version policy named, with the rejected alternatives and the reasons. DRAFT 2 superseded DRAFT 1, which was an outline, not a specification. This document is self-contained: it restates the context it needs from the kernel spec (v14) and the product spec (DRAFT 6) so that an engineer can read it, implement from it, and defend or attack any decision in it without another document open. Where it cites a requirement ID (F-, N-, D-) it also says what the requirement *is*. Its own sections carry T-numbers; the conformance suite cross-references both.
...
3. `fsync(dirfd)` — the rename itself is durable. A rename without a directory fsync is not durable; this is treated as law, not folklore.
...
### 6.3 Exceptions inside the cycle — the calls the skeleton dodged *(normative)*
...
- **The writer itself raises** (a kernel bug): crash loudly per §5.2's fsync-failure path. The record up to the last durable frame is intact and honest; a kernel that limps is a kernel that lies.
...
## 20. Conformance mapping and open questions
...
## 21. Composition internals *(normative)*
...
A spec that says "msgspec" once and "a vendored encoder" twice is not answering the question an implementer (or a reviewer) actually asks: *what, exactly, is this built from, and why each thing and not its rival?* This section is that answer. Policy first: the kernel's runtime dependency count is **one**. Everything else is stdlib, vendored, or fenced into extras and dev tooling. A runtime whose product is trust does not get to have a deep dependency tree — every transitive package is surface area for supply-chain risk (§17) and for silent behavior drift under upgrade (§4.1's reason for vendoring applies generally).
...
|---|---|---|
| RFC 8785 (JCS) encoder | ~120 lines | Conformance-critical: a dependency bump that changes one byte of canonical output silently invalidates every content hash and byte-identical-replay promise at once. Ships with RFC 8785's own test vectors in the conformance suite, plus Hypothesis property tests (§22.5). The PyPI `rfc8785` package was evaluated and is the fallback if the vendored encoder fails conformance — but the default posture is: we own the bytes |
| ULID generator | ~40 lines | run_ids need lexicographically-sortable, collision-free ids; the spec fits in a screen; a dependency for 40 lines is inverted economics |
...
| `hatchling` | Build backend, `pyproject.toml`-native | Boring on purpose; setuptools legacy and poetry's resolver opinions both rejected |
